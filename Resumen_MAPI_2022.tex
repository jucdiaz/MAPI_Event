...
\begin{enumerate}
	...
	\]
	Donde: $x_{i1}:$ Número de personas en habitar el inmueble, $x_{i2}:$ Tamaño del inmueble, $x_{i3}:$ Número de habitaciones del inmueble, $x_{i4}:$ Número de baños del inmueble;\\ en la tabla \ref{T_dise1} se describe los puntos de diseño seleccionados para las variables $x_{i2}, x_{i3}, x_{i4}$.
...
la variable respuesta esta definida como:
	...
	\[y_{j}=
		\begin{cases}
		1 & \text{si j-esima persona con una composición familiar $x_{i1}$ adquiere el inmueble}\\
...
Este paso se hizo con un tamaño $n$ de 100 y se hizo para una muestra de 16 expertos para cada punto de diseño.\\
...
\item Calcule el $N$ equivalente, esto permite cuantificar el conocimiento del experto en términos de tamaño muestral, este tamaño representa realmente el nivel de conocimiento que el experto tienen en la clasificación adecuada de inmuebles a una composición familiar determinada.

\[
N = \frac{4Z_{\alpha/2}^{2}\hat{\pi}(1-\hat{\pi})}{(\hat{\pi_S}-\hat{\pi_I})^{2}}
\]

\item Para cada nivel repita los siguientes pasos $m$ veces:\\
...
\item Genere un muestra de valores $y$ a partir de la distribución conjugada de la beta, la distribución binomial.
...
\keywords{Sistema de Recomendación, Inmubeles, Estadística Bayesiana, Redes Neuronales}
\end{mainabstract}
